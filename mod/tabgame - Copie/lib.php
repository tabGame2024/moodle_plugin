...
function tabgame_get_coursemodule_info($coursemodule) {
    ...
    if ($tabgame = $DB->get_record('tabgame', array('id'=>$coursemodule->instance), 'id, name, intro, introformat')) {
        if (empty($tabgame->name)) {
            // tabgame name missing, fix it
            $tabgame->name = "tabgame{$tabgame->id}";
            $DB->set_field('tabgame', 'name', $tabgame->name, array('id'=>$tabgame->id));
        }
        $info = new cached_cm_info();
        // no filtering hre because this info is cached and filtered later
        $info->content = format_module_intro('tabgame', $tabgame, $coursemodule->id, false);
        // $info->content = tabgame_get_content($tabgame);
...
function tabgame_dndupload_handle($uploadinfo) {
    ...
    if (!empty($uploadinfo->draftitemid)) {
        ...
        if ($file = reset($files)) {
            ...
            $data->intro = file_save_draft_area_files($uploadinfo->draftitemid, $context->id, 'mod_tabgame', 'intro', 0,
                                                      null, $data->intro);
        }
 ...
 */
function mod_tabgame_core_calendar_provide_event_action(calendar_event $event,
 ...
 * @return void
 */
function tabgame_get_content($tabgame){
    ...
    return format_text($tabgame->intro, $tabgame->introformat);
}
